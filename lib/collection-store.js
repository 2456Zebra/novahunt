...
class CollectionStore {
  ...
  getFilename(domain) {
    ...
    // and limit length to prevent filesystem issues
    const sanitized = domain.toLowerCase()
      .replace(/[^a-z0-9.-]/g, '_')
      .substring(0, 100);
    
    // Add hash to ensure uniqueness and prevent collisions
    const hash = crypto.createHash('sha256').update(domain.toLowerCase()).digest('hex').substring(0, 8);
