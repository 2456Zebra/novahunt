...
// NextAuth-compatible functions (using bcryptjs via existing hashedPassword field)
export async function findUserByEmail(email) {
  const store = readStore();
  const user = store.find(u => u.email === email);
  if (!user) return null;
  // Map hashedPassword to passwordHash for NextAuth compatibility
  return { ...user, passwordHash: user.hashedPassword };
}

export async function createUser(email, passwordHash, name = "") {
  const store = readStore();
  const id = randomUUID();
  // Store as hashedPassword internally for consistency with existing functions
  // passwordHash is exposed via findUserByEmail for NextAuth compatibility
  const user = { 
    id, 
    email, 
