...
async function createUser({ email, password, metadata = {} }) {
  ...
  const user = {
    ...
    createdAt: now,
    metadata,
    usage: { searchesUsed: 0, searchesTotal: 5, revealsUsed: 0, revealsTotal: 2 }
  };
...
async function getUsageForUser(userId) {
  ...
  return user.usage || { searchesUsed: 0, searchesTotal: 5, revealsUsed: 0, revealsTotal: 2 };
}
...
async function incrementUsage(userId, { searches = 0, reveals = 0 }) {
  ...
  if (!user.usage) {
    user.usage = { searchesUsed: 0, searchesTotal: 5, revealsUsed: 0, revealsTotal: 2 };
  }
