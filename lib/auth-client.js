...
//
// NOTE: updated setClientSignedIn behavior:
// - If the server returns a usage object that does NOT include a plan, we treat it as 'free' for client-side defaults.
//   This fixes the case where the UI showed "Reveals: 0 / 2" after signup; the client will now apply the free-plan defaults
//   (limitReveals = 3) when a plan is absent.
...
export function setClientSignedIn(email, usage = null) {
  try {
    ...
    if (usage) {
      // If server did not provide a plan, assume 'free' for client defaults so UI reflects expected limits
      const usageCopy = (typeof usage === 'string') ? JSON.parse(usage) : { ...usage };
      if (!usageCopy.plan) {
        usageCopy.plan = 'free';
      }
      // Normalize (applies plan defaults) before persisting
