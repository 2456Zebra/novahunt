...
function updateClientUsageSafely(updater) {
  try {
    const raw = getClientUsageRaw();
...
/**
 * incrementReveal
 * - Only increments if reveals < limitReveals
 * - Returns the updated usage object if increment succeeded
 * - Returns null if increment failed due to limit
 */
export function incrementReveal() {
  const raw = getClientUsageRaw();
  const usage = normalizeUsage(raw) || { plan: 'free', searches: 0, reveals: 0, limitSearches: 5, limitReveals: 3 };
  if (typeof usage.reveals !== 'number' || typeof usage.limitReveals !== 'number') return null;
  if (usage.reveals >= usage.limitReveals) {
    // do not increment beyond limit
    return null;
  }
  return updateClientUsageSafely((u) => ({ ...u, reveals: Number(u.reveals || 0) + 1 }));
}

/**
 * incrementSearch
 * - Only increments if searches < limitSearches
 * - Returns the updated usage object if increment succeeded
 * - Returns null if increment failed due to limit
 */
export function incrementSearch() {
  const raw = getClientUsageRaw();
  const usage = normalizeUsage(raw) || { plan: 'free', searches: 0, reveals: 0, limitSearches: 5, limitReveals: 3 };
  if (typeof usage.searches !== 'number' || typeof usage.limitSearches !== 'number') return null;
  if (usage.searches >= usage.limitSearches) {
    return null;
  }
  return updateClientUsageSafely((u) => ({ ...u, searches: Number(u.searches || 0) + 1 }));
}
