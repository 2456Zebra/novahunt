...
/*
 RightPanel — changes:
 - Removed the small numeric block under the description (no more "10 / 10 total" block)
 - Use enrichment.image > company.logo > Clearbit logo fallback
 - Show a single "Showing X of Y" line above the Upgrade CTA (if Y present)
 - Company link moved to the bottom-left under profile
*/
...
export default function RightPanel({ company }) {
  ...
  const enrichmentImage = (c && c.enrichment && c.enrichment.image) || null;
  const logoField = (c && c.logo) || null;
  ...
  const logoSrc = enrichmentImage || logoField || (domain ? getClearbitLogo(domain) : null);
      ...
      <div style={{ background: '#fff', border: '1px solid #e6edf3', borderRadius: 8, padding: 16 }}>
        <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            {/* Removed duplicate small domain line under title */}
          </div>
        </div>
        ...
        {/* Single "Showing X of Y" line above CTA (one line) */}
        <div style={{ marginTop: 12, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ color: '#6b7280', fontSize: 13 }}>
            {typeof total === 'number' && total > 0 ? `Showing ${shown} of ${total}` : `Showing ${shown} result${shown !== 1 ? 's' : ''}`}
          </div>
        ...
        {/* Company link at the very bottom-left under profile */}
