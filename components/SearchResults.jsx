...
// Placeholder company profile generator
const generateCompanyProfile = (domain) => {
  ...
  return {
    ...
    hq: 'Headquarters in San Francisco, CA',
    summary: 'This company has a strong reputation for innovation and market leadership. Known for delivering high-quality products and services.',
...
export default function SearchResults({ domain, results = [] }) {
  ...
  useEffect(() => setCompanyProfile(generateCompanyProfile(domain)), [domain]);
              ...
              {isExpanded && (
                <div className="mt-2 space-y-3">
                  {groupedResults[dept].map((r, idx) => {
                    const maskedEmail = r.email ? r.email.replace(/(^[^@]{1})[^@]*(@.*)$/, (m, a, b) => `${a}*****${b}`) : '';
                    return (
                      <div key={idx} className="flex justify-between items-center p-3 border rounded-lg">
                        <div>
                          <div className="font-medium">{r.first_name} {r.last_name}</div>
                          <div className="text-sm text-gray-600">{r.position}</div>
                          <div className="text-sm font-mono mt-1">{revealed.has(r.email) ? r.email : maskedEmail}</div>
                        </div>
                        <div className="flex flex-col items-end gap-1">
                          <span className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded">{r.score}% trust</span>
                          <div className="flex gap-1 mt-2">
                            <button onClick={() => setRevealed(s => new Set([...s, r.email]))} className="px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 text-sm">Reveal</button>
                            {r.source && <a href={r.source} target="_blank" rel="noopener noreferrer" className="px-3 py-1 border rounded text-sm hover:bg-gray-100">Source</a>}
                          </div>
                        </div>
                      </div>
                    );
