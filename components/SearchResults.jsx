...
const buildConversationalProfile = (r) => {
  ...
  if (r.name) {
    parts.push(`${r.name} is a${r.industry ? ` ${r.industry}` : ''} company${r.location ? ` based in ${r.location}` : ''}.`);
  }
  ...
  if (r.description) {
    ...
    // generics to make it feel modern-wiki-ish
    parts.push(
      `Today, ${r.name || 'the company'} focuses on delivering customer value through its products and services${
        r.website ? ` — learn more at ${r.website}` : '.'
      }`
    );
  }

  // Recent note or mission
  if (r.mission) {
    parts.push(`${r.name}’s mission: ${r.mission}`);
  } else if (r.recent) {
    parts.push(`Recent highlights: ${r.recent}`);
  }

  // Closing friendly line
    ...
    <>
      <style>
        {`
          ...
          .result-item:hover {
            ...
            box-shadow: 0 4px 10px rgba(19,20,21,0.04);
          }
      ...
      <div className="search-results-root" style={styles.container}>
        ...
        <div className="search-left" style={styles.left}>
          ...
          {results.length === 0 && (
            <div style={styles.noResults}>No results found. Try different keywords or filters.</div>
          )}
            ...
            >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
                ...
                <div style={styles.subtle}>
                  {result.industry ? result.industry + (result.location ? ` • ${result.location}` : '') : result.location || ''}
                </div>
              </div>
            </div>
          ))}
        ...
        >
          ...
          {!selected && (
            <div style={{ ...styles.noResults }}>
              Select a result on the left to see a modern, conversational company profile here.
            </div>
          )}
          ...
          {selected && (
            <div>
              <div style={styles.profileHeader}>
                {/* Logo */}
                {selected.logo || selected.logoUrl || selected.website ? (
                  <img
                    src={
                      selected.logo ||
                      selected.logoUrl ||
                      // try to guess a favicon if website available (may 404 gracefully)
                      (selected.website ? new URL(selected.website).origin + '/favicon.ico' : '')
                    }
                    alt={`${selected.name || 'Company'} logo`}
                    style={styles.logo}
                    onError={(e) => {
                      ...
                      placeholder.setAttribute('id', `ph-${Math.random().toString(36).slice(2)}`);
                    }}
                  />
                ) : (
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={styles.subtle}>
                    {selected.tagline || selected.shortDescription || (selected.industry ? `${selected.industry}` : '')}
                  </div>
                  ...
                  <div style={{ marginTop: 8 }}>
                    <a
                      ...
                      target={selected.website ? '_blank' : '_self'}
                      style={styles.websiteLink}
                    >
                      {selected.website ? new URL(selected.website).hostname.replace('www.', '') : 'No website available'}
                    </a>
                  </div>
                </div>
              </div>
              ...
              {/* Additional decorative / modern-wiki style sections */}
              <div style={{ marginTop: 16 }}>
                ...
                <div style={{ color: '#444' }}>
                  {selected.longDescription ? (
                    ...
                    <p style={{ marginTop: 0 }}>
                      This profile combines public-facing company information and curated context to give a quick,
                      modern overview. If you have a specific question about {selected.name}, try searching for it directly
                      or clicking through to the source site.
                    </p>
                  )}
                </div>
              </div>
