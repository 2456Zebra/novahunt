            ...
            <div style={{ maxWidth:760 }}>
              ...
              <div style={{ display:'flex', gap:12, alignItems:'center', marginBottom:14 }}>
                <div style={{ flex:1, display:'flex', alignItems:'center', background:'#fff', borderRadius:8, border:'1px solid #e6edf3', padding:6 }}>
                  <input aria-label="domain" value={domain} onChange={e => setDomain(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') loadDomain(domain); }} placeholder="Enter domain, e.[...]" style={{ width: '100%', border: 'none', outline: 'none', padding: '10px 12px', fontSize: 16 }} />
                </div>

                <button onClick={() => loadDomain(domain)} style={{ background:'#2563eb', color:'#fff', border:'none', padding:'10px 14px', borderRadius:6, fontWeight:700, cursor:'pointer' }}>{loading ? 'Searching…' : 'Search'}</button>
              </div>
              ...
              <div style={{ color:'#6b7280', fontSize:13, marginBottom:12 }}>
                ...
                <div style={{ marginTop:8, display:'flex', gap:12, flexWrap:'wrap' }}>
                  {SAMPLE_DOMAINS.map(d => (<a key={d} href="#" onClick={(e)=>{e.preventDefault(); loadDomain(d);}} style={{ fontSize:13, color: '#2563eb', textDecoration: 'underline' }}>{d}</a>))}
                </div>
              </div>
            </div>
