...
export default function ForgotPassword() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (res.ok) {
        setMsg({ type: 'success', text: (json && json.message) ? json.message : 'If an account exists you will receive an email.' });
      } else if (res.status === 404) {
        setMsg({ type: 'info', text: 'Forgot-password API not implemented yet.' });
      } else {
      ...
      setMsg({ type: 'error', text: 'Unable to process request. Check console.' });
    ...
    <div style={{ fontFamily: 'sans-serif' }}>
      ...
      <main style={{ maxWidth: 680, margin: '32px auto', padding: 20 }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" disabled={loading} style={{ padding: '8px 14px' }}>{loading ? 'Sending...' : 'Send reset link'}</button>
        </form>
