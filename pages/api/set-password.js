 ...
 *
 * SECURITY:
 * - Do NOT rely on Stripe session_id as a long-lived auth token in production.
 * - Use JWT_SECRET in Vercel environment to sign tokens.
 * - If using Supabase, set SUPABASE_SERVICE_ROLE_KEY in Vercel (server-only).
 */
export default async function handler(req, res) {
  ...
  // Placeholder for token verification (one-time tokens)
  if (token) {
    // TODO: verify token and set email
    // const payload = verifyOneTimeToken(token);
    // if (!payload || !payload.email) return res.status(400).json({ error: 'Invalid token' });
    // email = payload.email;
  }

  // Fallback: use Stripe Checkout session to get customer email
  ...
  try {
    ...
    if (supabase) {
      // Upsert user in Supabase users table (requires service role key)
      try {
        const upsertPayload = {
          email,
      ...
      // Demo fallback: issue cookie and continue without persistence
