...
// Normalizes Supabase admin list responses that return { users: [...] } or an array.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Normalization: Supabase sometimes returns an object { users: [...] }
    if (users && users.users && Array.isArray(users.users)) {
      users = users.users;
    }

    if (listResp.ok) {
      // Now support either an array or a single user object
      ...
      if (first) {
        ...
        if (userId) {
          ...
          // ID not discovered OR not a UUID -> try email-path fallback
    ...
    if (isEmailExists) {
      ...
      if (users2 && users2.users && Array.isArray(users2.users)) users2 = users2.users;
