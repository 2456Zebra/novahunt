...
// Aggressive-retry, verbose-logging finalize endpoint for new accounts.
// - Retries admin lookup/update longer when create returns email_exists (handles eventual consistency).
// - Logs truncated responses so you can inspect Vercel function logs.
// - Sets session cookies after successful create/update via token exchange (uses SUPABASE_ANON_KEY).
// - Honors DISABLE_PASSWORD_RECOVER to stop sending recovery emails while remediating bounces.
//
...
// - STRIPE_SECRET_KEY
// - POST_RESET_REDIRECT
// - DISABLE_PASSWORD_RECOVER (set "1" to disable sending recover emails)
...
async function findUserByEmail(email) {
  ...
  try {
    ...
    console.info('findUserByEmail result (trunc):', Array.isArray(js) ? `array(${js.length})` : (js && js.id ? 'object_with_id' : 'no_user'));
...
async function createUser(email, password) {
  ...
  try {
    ...
    console.info('createUser succeeded (trunc)', js.id ? 'created user id present' : 'created user');
...
async function updateUserPassword(uid, password) {
  ...
  try {
    ...
    const js = JSON.parse(txt || '{}');
    console.info('updateUserPassword succeeded (trunc)', js.id ? 'updated user id present' : 'updated user');
    return { ok: true, user: js };
  } catch (err) {
...
async function sendPasswordRecover(email) {
  ...
  try {
    const res = await fetch(url, { method: 'POST', headers, body: JSON.stringify(body) });
    const txt = await res.text().catch(()=>null);
    if (res.ok) return { ok: true, body: txt };
...
async function signInAndSetCookies({ email, password, req, res }) {
  ...
  try {
    ...
    console.info('signInAndSetCookies: cookies set (trunc)');
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (found.ok && found.user) {
      ...
      if (uid) {
        ...
        if (!updated.ok) {
          ...
          if (recover.status === 429) return json(res, 200, { ok: true, action: 'password_reset_rate_limited', retry_after: recover.retryAfter || 40 });
          if (recover.body === 'recover_disabled') return json(res, 200, { ok: true, action: 'password_reset_disabled', message: 'Password recovery temporarily disabled' });
        ...
        if (!signed.ok) return json(res, 200, { ok: true, action: 'updated', warning: 'session_not_created', details: signed.body || signed });
    ...
    if (created.ok) {
      const signed = await signInAndSetCookies({ email, password, req, res });
      if (!signed.ok) return json(res, 200, { ok: true, action: 'created', warning: 'session_not_created', details: signed.body || signed });
      return json(res, 200, { ok: true, action: 'created', session: signed.session || null });
    }

    // 3) create returned error -> check for email_exists and retry lookup/update (longer)
    const createErrCode = parseCreateError(created.body || created.error || created);
    if (createErrCode === 'email_exists') {
      console.warn('createUser reported email_exists - retrying admin lookup/update with backoff (attempts=8)');
      const attempts = 8;
      for (let i = 0; i < attempts; i++) {
        await sleep(750 * (i+1)); // increasing delay: 750ms, 1500ms, 2250ms...
        ...
        if (f.ok && f.user) {
          ...
          if (uid2) {
            ...
            if (updated2.ok) {
              ...
              if (!signed2.ok) return json(res, 200, { ok: true, action: 'updated_after_create_conflict', warning: 'session_not_created', details: signed2.body || signed2 });
              return json(res, 200, { ok: true, action: 'updated_after_create_conflict', session: signed2.session || null });
            } else {
              console.warn('updateUserPassword after retry failed (trunc):', updated2.body || updated2);
            }
          }
        }
      }
      // exhausted retries -> fallback to recover (respect disabled & rate-limits)
      const recover = await sendPasswordRecover(email);
      if (recover.ok) return json(res, 200, { ok: true, action: 'password_reset_sent', message: 'Password recovery email sent' });
      if (recover.status === 429) return json(res, 200, { ok: true, action: 'password_reset_rate_limited', retry_after: recover.retryAfter || 40 });
      if (recover.body === 'recover_disabled') return json(res, 200, { ok: true, action: 'password_reset_disabled', message: 'Password recovery temporarily disabled' });
